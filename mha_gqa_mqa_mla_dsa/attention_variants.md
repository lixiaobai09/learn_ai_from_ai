...
$$W_i^Q,__;__W_i^K,__;__W_i^V \\in \\mathbb{R}^{d_\\text{model} \\times d\_\\text{head}} = \\mathbb{R}^{768 \\times 64}$$
...
$$K = c_{KV}__,__W^{UK},\\quad V = c_{KV},W^{UV}$$
...
$$Q,K^\\top = (c_Q__,__W^{UQ})__,__(c_{KV},W^{UK})^\\top = c_Q__,__W^{UQ}__,__(W^{UK})^\\top__,__c_{KV}^\\top$$
...
$$\\text{score}_i = c_Q__,__\\widetilde{W}\_i__,__c_{KV}^\\top$$
...
$$\\text{MHA} = \\sum_i \\text{head}\_i__,__W_i^O = \\sum_i A_i__,__c_{KV},W_i^{UV},W_i^O$$
...
$$\\text{MHA} = \\sum_i A_i__,__c_{KV},\\widehat{W}\_i^O$$
...
$$\\begin{pmatrix} x'_{2i}__\\__ x'_{2i+1}\\end{pmatrix}
= \\begin{pmatrix}\\cos(m\\theta_i) & -\\sin(m\\theta_i)__\\__ \\sin(m\\theta_i) & \\cos(m\\theta_i)\\end{pmatrix}
\\begin{pmatrix} x_{2i}\\ x\_{2i+1}\\end{pmatrix}$$
...
$$\\text{score}_i = c_Q__,__\\widetilde{W}\_i__,__c_{KV}^\\top,\\quad \\widetilde{W}\_i = W_i^{UQ}(W_i^{UK})^\\top$$
...
$$\\text{score}_i = c_Q__,__\\underbrace{W_i^{UQ}(W_i^{UK})^\\top}_{\\widetilde{W}_i \\text{ 是常量}}__,__c_{KV}^\\top$$

中间是**位置无关的固定矩阵**，可以预算好。

加上 RoPE 后：

$$\\text{score}_i = (R_m Q_i)^\\top (R_n K_i) = Q_i^\\top R_{n-m} K_i = c_Q__,__W_i^{UQ}__,__R_{n-m},(W_i^{UK})^\\top__,__c_{KV}^\\top$$
...
$$\\boxed{;\\text{score}_i __;__=__;__ \\underbrace{Q_i^C \\cdot (K_i^C)^\\top}_{\\text{content 项}} ;+; \\underbrace{Q_i^R \\cdot (K^R)^\\top}_{\\text{RoPE 项}};}$$

**两项的计算特点完全不同：**

**content 项**——完美吸收，和纯 MLA 一样：

$$Q_i^C__,__(K_i^C)^\\top = c_Q__,__W_i^{UQ}(W_i^{UK})^\\top c_{KV}^\\top = c_Q__,__\\widetilde{W}\_i__,__c_{KV}^\\top$$
...
$$\\text{out}_t = \\text{MLA-Attention}__!__\\left(Q_t,__,__{__K_s, V_s__}_{s \\in S_t}\\right)$$
...
* **MHA**: Vaswani et al., _Attention Is All You Need_, 2017.
* **MQA**: Shazeer, _Fast Transformer Decoding: One Write-Head is All You Need_, 2019.
* **GQA**: Ainslie et al., _GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints_, 2023.
* **MLA**: DeepSeek-AI, _DeepSeek-V2: A Strong, Economical, and Efficient Mixture-of-Experts Language Model_, 2024.
* **DSA**: DeepSeek-AI, _DeepSeek-V3.2-Exp: Boosting Long-Context Efficiency with DeepSeek Sparse Attention_, 2025.
